...
const downloadPdf = async (source: SourceMetadata): Promise<ExportResult> => {
  try {
    ...
    const writeBlock = (
      text: string,
      options: { fontSize?: number; bold?: boolean; after?: number } = {},
    ) => {
    ...
    writeBlock(source.title, { fontSize: 20, bold: true, after: 8 });
    writeBlock('Generated by Capyap', { fontSize: 10, after: 2 });
    writeBlock(
      `Words: ${source.wordCount.toLocaleString()} | Segments: ${source.segments.length}`,
      { fontSize: 10, after: 4 },
    );
    if (source.url) {
      writeBlock(`Source: ${source.url}`, { fontSize: 10, after: 10 });
    }

    if (source.chapters.length > 0) {
      writeBlock('Chapters', { fontSize: 14, bold: true, after: 4 });
      for (const chapter of source.chapters) {
        writeBlock(`- [${formatTime(chapter.start)}] ${chapter.title}`, {
          fontSize: 11,
          after: 1,
        });
      }
    ...
    writeBlock('Transcript', { fontSize: 14, bold: true, after: 4 });
    for (const segment of source.segments) {
      writeBlock(`[${formatTime(segment.start)}] ${segment.text}`, {
        fontSize: 11,
        after: 4,
      });
    }
...
const buildExportFilename = (source: SourceMetadata, extension: string): string => {
  return `${source.title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_transcript.${extension}`;
};
